...
|---|---|---|---|
| **CP63** | Guard/router tiering + the four subagents | Tier 1 answers in ~1s; multi-agent **measurably** beats CP61's baseline on the golden set (if it does not, we say so and keep the baseline) | **merged and deployed** (PR #110), re-verified live. Measured against real questions, not the golden set (CP65 doesn't exist yet): tier 3 (web) is a genuine new capability and works correctly. Tier 2 does NOT measurably beat the baseline — a live comparative question made `stats-scout` issue ten redundant tool calls and still hadn't converged after 287s, against CP61's 50.9s for the same class — so per this row's own escape clause, tier 2 was downgraded to route through CP61's flat graph instead. See `HANDOFF.md` for the full writeup. |
| **CP64** | Verifier node, citation contract, repair loop, framing contracts | Every factual claim carries a resolvable citation; classes 10-11 framed correctly; forced-failure test proves the repair loop fires | **merged and deployed.** Deterministic core only (citation existence, number-matching, predictive/subjective framing) — LLM claim-extraction and entailment passes deferred, same "measure before adding the expensive part" call CP63 made. Live-verified: a real answer cited `[ev_1]` correctly and passed with zero violations; the repair loop is proven via a stubbed agent (`test_agent_graph.py`'s `RepairLoopTests`), not a live call. Two harder live questions hit the pre-existing recursion-limit/timeout degrade paths instead of converging — not a CP64 regression, the same tool-call reliability risk this batch's own retrospective already named. See `HANDOFF.md`. |
| **CP65** | deepeval golden set (~60 cases) + CI gate; LangSmith datasets + feedback wiring | Deterministic metrics gate every PR; nightly judged run; thumbs-up/down reaches LangSmith | **merged.** Scoped down: 24 authored cases (no real production traces exist yet to mine 60 from), and the CI gate is `router.classify`/`verifier.check` run deterministically against every case (genuinely gates every PR, no model call) rather than `deepeval`'s live-trace metrics, which would need a real Ollama call per case on every PR — a cost this free-tier architecture is built to avoid. `deepeval` itself is wired and smoke-tested (fabricated traces) but not installed in the shared dev sandbox. LangSmith dataset curation and feedback UI deferred — no real traces yet, no frontend time this checkpoint. See `HANDOFF.md`. |
| **CP66** | Production UI + hardening: portaled panel, activity timeline, sources, cancel, rate limits, budget caps, answer cache, load test | Driven on the **real deployed site**, per Batch 16's lesson | **merged and deployed.** Portaled panel + activity timeline + answer cache shipped; budget caps and a synthetic load test deferred to a future batch (real product-policy decisions and real traffic to calibrate against, not silently dropped — see `HANDOFF.md`). Verified live on the deployed site per this row's own done-criterion. |
