...
//
// Scope is a deliberate ~15 lineages / ~40 nodes / ~25 rename edges, not all
...
// (Ferrari, McLaren, Williams, Brabham, classic Lotus, Cooper, BRM, Vanwall)
// are included as one-node lineages specifically for visual contrast against
// the ones that kept changing names — per the CP49 brief.
//
...
export function resolveLineages(
  ...
  return CONSTRUCTOR_LINEAGES.map((lineage) => ({
    ...
    nodes: lineage.nodes.map((node) => {
      ...
      if (years.length === 0) {
        return { ...node, startYear: null, endYear: null, invalid: true };
      }
